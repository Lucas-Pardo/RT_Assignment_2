...
\end{DoxyItemize}\hypertarget{md__r_e_a_d_m_e_autotoc_md3}{}\doxysection{How to run the program}\label{md__r_e_a_d_m_e_autotoc_md3}
...
The values {\ttfamily (R)}, {\ttfamily (GX)} and {\ttfamily (GY)} are the user input. The launch file is explained later but the {\ttfamily rate} argument has a default value so {\ttfamily rate\+:=(R)} can be omitted.\hypertarget{md__r_e_a_d_m_e_autotoc_md4}{}\doxysection{In-\/depth explanation of the code}\label{md__r_e_a_d_m_e_autotoc_md4}
Every cpp file is adecuately commented to understand what is being done but here we provide a more detailed explanation and the motive behind every decision.\hypertarget{md__r_e_a_d_m_e_autotoc_md5}{}\doxysubsection{Node A (move\+\_\+to\+\_\+goal.\+cpp)}\label{md__r_e_a_d_m_e_autotoc_md5}
Setting the goal is easy and can be done just by passing the goal position as argument to the function ({\itshape argv}). This can be parsed and sent to the action server. However, we also must be able to cancel the goal at any time, which means that we need to check for user input periodically. We must also publish the custom message with the given rate.\hypertarget{md__r_e_a_d_m_e_autotoc_md6}{}\doxysubsubsection{$<$strong$>$\+Pseudocode$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md6}
...
Our code is a little different in the sense that it cycles itself after finishing to allow the user to input a new goal, so in reality, this pseudocode would be inside a loop.\hypertarget{md__r_e_a_d_m_e_autotoc_md7}{}\doxysubsubsection{$<$strong$>$\+Argument parsing$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md7}

\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordtype}{double} goal\_x, \mbox{\hyperlink{robot__status_8cpp_a86a424410ae6b4f43baca838da5f993b}{goal\_y}};}
...
\DoxyCodeLine{    goal\_x = atof(argv[2]);}
...
\DoxyCodeLine{    goal\_x = atof(argv[1]);}
...
This is not really useful for the complete execution of the assignment (using the launch file) since it will always have the three arguments, else the launch itself fails. Nevertheless, it is useful when the node is run manually.\hypertarget{md__r_e_a_d_m_e_autotoc_md8}{}\doxysubsubsection{$<$strong$>$\+Checking user input$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md8}
...
We take advantage of the function {\ttfamily wait\+For\+Result}, which can take as argument a duration to wait for, to make the program sleep for the given rate.\hypertarget{md__r_e_a_d_m_e_autotoc_md10}{}\doxysubsubsection{$<$strong$>$\+Publisher and subscriber$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md10}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordtype}{float} x, y, vel\_x, \mbox{\hyperlink{move__to__goal_8cpp_a1dd0876197a1e2dba35d41d6a6d9eab4}{vel\_y}};}
\DoxyCodeLine{}
...
\DoxyCodeLine{    x = (float) msg-\/>pose.pose.position.x;}
\DoxyCodeLine{    y = (\textcolor{keywordtype}{float}) msg-\/>pose.pose.position.y;}
\DoxyCodeLine{    vel\_x = (float) msg-\/>twist.twist.linear.x;}
...
\end{DoxyCode}
\hypertarget{md__r_e_a_d_m_e_autotoc_md11}{}\doxysubsubsection{$<$strong$>$\+Information about the goals$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md11}
...
\DoxyCodeLine{\mbox{\hyperlink{goals__status_8cpp_a898856750d65dc6e6706ea4772115d04}{nh}}.setParam(\textcolor{stringliteral}{"/goals\_cancelled"}, gc + 1);}
\end{DoxyCode}
\hypertarget{md__r_e_a_d_m_e_autotoc_md12}{}\doxysubsubsection{$<$strong$>$\+Exit routine$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md12}
...
\DoxyCodeLine{        std::cin >> buf;}
\DoxyCodeLine{        goal\_x = atof(buf.c\_str());}
...
\DoxyCodeLine{cycle:}
\DoxyCodeLine{}
\DoxyCodeLine{goal.target\_pose.pose.position.x = goal\_x;}
...
\end{DoxyCode}
\hypertarget{md__r_e_a_d_m_e_autotoc_md13}{}\doxysubsection{Node B (goals\+\_\+status.\+cpp)}\label{md__r_e_a_d_m_e_autotoc_md13}
...
\DoxyCodeLine{-\/-\/-\/}
\end{DoxyCode}
\hypertarget{md__r_e_a_d_m_e_autotoc_md14}{}\doxysubsubsection{$<$strong$>$\+Other possible approaches$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md14}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{md__r_e_a_d_m_e_autotoc_md17}{}\doxysubsubsection{$<$strong$>$\+Callback function$<$/strong$>$}\label{md__r_e_a_d_m_e_autotoc_md17}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordtype}{float} x, y, vel\_x, \mbox{\hyperlink{move__to__goal_8cpp_a1dd0876197a1e2dba35d41d6a6d9eab4}{vel\_y}}, goal\_x = 0, \mbox{\hyperlink{robot__status_8cpp_a86a424410ae6b4f43baca838da5f993b}{goal\_y}} = 0;}
\DoxyCodeLine{\textcolor{keywordtype}{int} \mbox{\hyperlink{robot__status_8cpp_a4e1e0e72dd773439e333c84dd762a9c3}{c}} = 0;}
\DoxyCodeLine{\textcolor{keywordtype}{float} acc\_vx = 0, \mbox{\hyperlink{robot__status_8cpp_a49e8ddc680bdec2d0944af604628b5df}{acc\_vy}} = 0;}
\DoxyCodeLine{}
...
\DoxyCodeLine{    \textcolor{keywordflow}{if} (std::abs(msg-\/>goal\_x -\/ goal\_x) > eps || std::abs(msg-\/>goal\_y -\/ \mbox{\hyperlink{robot__status_8cpp_a86a424410ae6b4f43baca838da5f993b}{goal\_y}}) > eps) \{}
\DoxyCodeLine{        ROS\_INFO(\textcolor{stringliteral}{"Goal change detected, resetting stats."});}
\DoxyCodeLine{        goal\_x = msg-\/>goal\_x;}
\DoxyCodeLine{        \mbox{\hyperlink{robot__status_8cpp_a86a424410ae6b4f43baca838da5f993b}{goal\_y}} = msg-\/>goal\_y;}
\DoxyCodeLine{        acc\_vx = 0;}
...
\DoxyCodeLine{    x = msg-\/>x;}
\DoxyCodeLine{    y = msg-\/>y;}
\DoxyCodeLine{    vel\_x = msg-\/>vel\_x;}
\DoxyCodeLine{    \mbox{\hyperlink{move__to__goal_8cpp_a1dd0876197a1e2dba35d41d6a6d9eab4}{vel\_y}} = msg-\/>vel\_y;}
\DoxyCodeLine{    acc\_vx += std::abs(vel\_x);}
...
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"x = "} << x << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"y = "} << y << std::endl;}
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"vel\_x = "} << vel\_x << std::endl;}
...
\DoxyCodeLine{    std::cout << \textcolor{stringliteral}{"Distance to goal: "} << std::sqrt((goal\_x -\/ x)*(goal\_x -\/ x) + (\mbox{\hyperlink{robot__status_8cpp_a86a424410ae6b4f43baca838da5f993b}{goal\_y}} -\/ y)*(\mbox{\hyperlink{robot__status_8cpp_a86a424410ae6b4f43baca838da5f993b}{goal\_y}} -\/ y)) << std::endl;}
\DoxyCodeLine{    \textcolor{keywordtype}{float} av\_x = acc\_vx / \mbox{\hyperlink{robot__status_8cpp_a4e1e0e72dd773439e333c84dd762a9c3}{c}};}
...
\end{DoxyItemize}\hypertarget{md__r_e_a_d_m_e_autotoc_md19}{}\doxysection{Improvements}\label{md__r_e_a_d_m_e_autotoc_md19}
